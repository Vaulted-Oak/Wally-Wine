        ...
        <div>
          ...
          <div className="grid items-start gap-12 sm:grid-cols-1 sm:divide-y sm:divide-gray-200 md:grid-cols-4 md:gap-16">
                  ...
                  <tr key={lineItem.id}>
                    <td className="w-full max-w-0 py-4 pl-0 pr-3 align-top sm:w-auto sm:max-w-none sm:align-middle">
                      <div className="flex gap-6">
                        {lineItem?.image && (
                          <div className="card-image aspect-square w-12 h-12 min-w-12 flex-shrink-0">
                            <Image
                              data={lineItem.image}
                              height={96}
                              width={96}
                        ...
                        <dl className="grid">
                          ...
                          <dd className="truncate lg:hidden">
                            <h3 className="truncate lg:hidden text-xs md:text-sm">{lineItem.title}</h3>
                            <p className="mt-1">{lineItem.variantTitle}</p>
                          </dd>
                          <dt className="sr-only">
                            {themeContent?.account.price}
            ...
            <div className="top-nav sticky border-none md:my-8">
              <h3 className="text-2xl">{themeContent?.account.shippingAddress}</h3>
              {order?.shippingAddress ? (
                <ul className="mt-6">
                  <li className="pb-2">{order.shippingAddress.name}</li>
                  {order?.shippingAddress?.formatted ? (
                    order.shippingAddress.formatted.map((line: string) => (
                      <li className="pb-2" key={line}>{line}</li>
                    ))
                  ) : (
                    <></>
                  )}
              ...
              <h3 className="mt-8 mb-4 text-2xl">Status</h3>
              {fulfillmentStatus && (
                <span>
                <Badge className="pt-4 pb-4 w-[100px] text-[14px]"
