...
export function createSanityClient(options: CreateSanityClientOptions) {
  ...
  const sanity: Sanity = {
    ...
    async query({
      ...
      const queryHash = await hashQuery(query, params, queryOptions);
      ...
      return withCache.run(
        ...
        async () => {
          // Use the native Sanity client fetch method
          const data = await client.fetch(query, params || {}, queryOptions);
...
function hashQuery(
  ...
  params?: QueryParams,
  queryOptions?: { perspective?: string },
) {
  let hash = query;
  ...
  if (queryOptions?.perspective) {
    hash += queryOptions.perspective;
  }
