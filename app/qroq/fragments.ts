...
export const CMS_MENU_FRAGMENT = q(
  `coalesce(
      cmsMenu[_key == $language][0].value[],
      cmsMenu[_key == $defaultLanguage][0].value[],
    )[]`,
  { isArray: true },
)
  .select(LINKS_LIST_SELECTION)
...
/*
|--------------------------------------------------------------------------
| Header Selected Pages Fragments
|--------------------------------------------------------------------------
*/

export const PAGE_FRAGMENT = {
  title: q.array(q.object({
    value: q.string(),
    _key: q.string(),
    _type: q.string()
  })),
...
export const HEADER_SELECTED_PAGES_FRAGMENTS = q('selectedPages[]', { isArray: true }).deref().grab(PAGE_FRAGMENT).nullable()
...
export const CMS_ANNOUNCEMENT_BAR_ARRAY_FRAGMENT = q(
  `coalesce(
      cmsAnnouncementBar[_key == $language][0].value[],
      cmsAnnouncementBar[_key == $defaultLanguage][0].value[],
    )[]`,
  { isArray: true },
)
  .select({
