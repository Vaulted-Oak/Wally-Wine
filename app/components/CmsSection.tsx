...
import type { EncodeDataAttributeCallback } from '~/lib/sanity/types';
...
type CmsSectionsProps =
  | (NonNullable<InferType<typeof COLLECTION_SECTIONS_FRAGMENT>>[0] & {
    settings?: InferType<
      typeof SECTION_SETTINGS_FRAGMENT
    >
  })
...
function SectionWrapper(props: {
  ...
  const css = data?.settings?.customCss?.code;
  const customCss = css ? css.replace(/(^|\s)(\.[^{,]+)/g, `$1${sectionSelector} $2`) : '';
