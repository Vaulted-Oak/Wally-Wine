...
>(({className, ...props}, ref) => (
  <div
    className={cn('private-dining-banners absolute inset-0 overflow-hidden', className)}
    ref={ref}
    {...props}
  >
...
>(({className, contentAlignment, contentPosition, ...props}, ref) => {
  ...
  return (
    <div
      className={cn(
        'relative z-[3] flex h-full py-4 private-dining-content',
        contentAlignmentVariants({
      ...
      {...props}
    >
      <div className={cn('max-w-[40rem] unset-max-width')}>{props.children}</div>
    </div>
  );
});
