...
export function CartLines({
  ...
  const className = cx([
    layout === 'page'
      ? 'flex-grow md:translate-y-4 lg:flex-[0_0_70%]'
      ...
      <section aria-labelledby="cart-contents" className={className}>
        <ul className="grid">
          {currentLines.map((line) => (
            <li key={line.id}>
              <CartLineItem itemsCount={currentLines.length} layout={layout} line={line as UpdatedCartLineFragment} onClose={onClose} />
            </li>
          ))}
        </ul>
      </section>
