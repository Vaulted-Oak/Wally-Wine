...
// Custom SSR-safe hooks to replace react-use
...
import { Cart, Product } from '@shopify/hydrogen/storefront-api-types';
...
export function AddToCartForm(props: {
  ...
  useEffect(() => {
    CheckMetafieldConditions({cart,product,setErrorMessage});
  }, [cart, product]);

  const cartHasFutures = cart?.lines?.nodes?.some(
    (line) => line?.merchandise?.product?.futures?.key === "special_designation" && line?.merchandise?.product?.futures?.value === "Futures"
  );
  const productHasFutures = product?.futures?.key === "special_designation" && product?.futures?.value === "Futures";
        ...
        <div className="mt-4 w-full md:mt-0">
          ...
          >
            {(fetcher) => {
              ...
              useEffect(() => { if (warning && fetcher.state === 'loading'){setShowWarningMessage(true); setWarningMessage(warning)} }, [warning,fetcher.state]);
                ...
                <div className="grid gap-3">
                  ...
                  <Button
                    className={cn([
                      ...
                      'uppercase',
                      'font-medium', // Customize padding for x and y axes
                    ])}
          ...
          {cart?.lines?.nodes && cart?.lines?.nodes?.length > 0 && productHasFutures && !cartHasFutures ? (
            <span className="mt-4 block text-primaryRed">*You must order Futures separately.</span>
          ): null}
        </div>
      </>
    )
...
export function ErrorMessageSection(props: {
  errorMessage: string;
  ...
  return (
    <div>
      <div className="fixed left-0 top-0 z-[1055] h-full w-full transform-none !overflow-hidden overflow-y-auto overflow-x-hidden outline-none">
        <div className="flex items-center pointer-events-none pointer-events-auto relative h-[90vh] w-auto translate-y-0 overflow-auto opacity-100 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
          <div className="ml-2 mr-2 mt-4 item-center justify-center min-h-[150px] min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-auto flex-col rounded-md border-none bg-white bg-clip-padding p-8 text-current shadow-lg outline-none dark:bg-neutral-600">
            <span className="flex flex-wrap item-center justify-center mb-4 center">
              <svg fill="#ab2c2c" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
            ...
            <button
              ...
              className="absolute right-2 top-2 h-8 w-8 bg-transparent"
            >
              <svg
                width="22"
                height="20"
...
export function CheckMetafieldConditions(props: {
  cart: Cart | null;
  product?: Product;
  setErrorMessage: (value: string | null) => void;
}){
  if (!props?.cart?.lines?.nodes?.length) {
    // If the cart is empty, do not display any error message
    props?.setErrorMessage(null);
    return;
  }
  // const cartHasFutures = props?.cart.lines.nodes.some(
  //   (line) =>
  //     line?.merchandise.product?.title?.toUpperCase().includes('FUTURES')
  // );
  const cartHasFutures = props?.cart.lines.nodes.some(
    (line) => line?.merchandise?.product?.futures?.key === "special_designation" && line?.merchandise?.product?.futures?.value === "Futures"
  );
  // const productHasFutures =
  //   props?.product?.title?.toUpperCase().includes('FUTURES');
  const productHasFutures = props?.product?.futures?.key === "special_designation" && props?.product?.futures?.value === "Futures";

  if (cartHasFutures && !productHasFutures) {
    return props.setErrorMessage(
      'You must order Futures separately. Please remove one of the item types from your cart.',
    );
  } else if (!cartHasFutures && productHasFutures) {
    return props.setErrorMessage(
      'You must order Futures separately. Please remove one of the item types from your cart.',
    );
  } else {
    return props.setErrorMessage(null);
  }
};
...
function ShopPay(props: {
  ...
  const rootData = useRootLoaderData();
  const {env, cart} = rootData || {};
  const [show, setShow] = useState<boolean>(false);
  const [userIsIdle, setUserIsIdle] = useState(true);
  ...
  useEffect(() => {
    ...
    const shopPayLoaded = sessionStorage.getItem('shop-pay-state') === 'true';
    if (shopPayLoaded) {
      setShow(true);
      return;
    }
    ...
    const resetIdleTimer = () => {
      ...
      idleTimer = setTimeout(() => setUserIsIdle(true), 1000);
    };
  ...
  useEffect(() => {
    ...
    if (!userIsIdle && !show) {
      setShow(true);
      sessionStorage.setItem('shop-pay-state', 'true');
    }
  }, [userIsIdle, show]);
