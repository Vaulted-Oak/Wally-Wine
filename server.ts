...
export async function fetch(
  ...
  try {
    ...
    // 1. Session has preview mode set (manual toggle via keyboard shortcut)
    // 2. Request has sanity preview headers (from Sanity Studio)
    const hasPreviewSession = await sanitySession.has('previewMode');
    const hasSanityHeaders = request.headers.get('x-sanity-perspective') === 'previewDrafts' || 
                             request.headers.get('x-sanity-preview') === 'true';
    const sanityPreviewMode = hasPreviewSession || hasSanityHeaders || 
                              (envVars.SANITY_STUDIO_USE_PREVIEW_MODE === 'true' && isDev);
